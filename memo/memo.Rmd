...
coord <- storms|>
  ...
  select(BEGIN_LAT, BEGIN_LON, BEGIN_YEAR, STATE)
view(coord)
```


```{r take_2, eval=FALSE}
counties <- counties(state = NULL, cb = TRUE, class = "sf")

coord_sf <- st_as_sf(coord, coords = c("BEGIN_LON", "BEGIN_LAT"), crs = st_crs(us_counties_sf))

tornados_map <- st_join(coord_sf, counties, join = st_intersects)
```


```{r point_count, eval=FALSE}
real_county_count <- tornados_map|> 
  count(GEOID, name = "point_count")

county_map <- map_data('county')|>
  mutate(county_fips_name = paste(region, subregion, sep = ","))

```


```{r leaflet_tornado_density, eval=FALSE}
ggplot(data=county_map, aes(x=long, y=lat, fill=region, group=group)) + 
