...
```{r point_count, message=FALSE, warning=FALSE, message=FALSE}
...
geoid_df <- counties(state = NULL, cb = TRUE, progress_bar = FALSE)|>
  ...
  #I CHECKED THIS IS NOT THE COUNTY ISSUE
...
all_coords_geoid <- left_join(geoid_df, tornados_per)
...
#adds number of tornados to map df
...
ggplot(data=map_n, aes(x=long, y=lat, fill = T_count, group = group))+
  ...
  labs(
    title = "Number of tornados per county 2010-2020",
    fill = "Quantity"
